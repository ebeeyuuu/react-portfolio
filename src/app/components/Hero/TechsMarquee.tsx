...
const TechsMarquee = () => {
  ...
  return (
    <div
      data-aos="fade-up"
      data-aos-delay="650"
      className="flex flex-col justify-center items-center max-w-[500px] max-sm:max-w-[400px] max-[1000px]:mt-[200px]"
    >
      <div className="text-xl font-semibold mb-[30px] mt-[-50px] max-[1000px]:mt-[20px]">
        What I offer:
      </div>
      <Marquee
        className="gap-[3rem] max-w-[500px] max-sm:max-w-[400px] border-white border-b-2 border-t-2 p-2 py-5 smooth-animation"
        autoFill={true}
      >
        {marqueeItems.map((item, index) => (
          <div key={index} className="flex items-center space-x-5">
            <div className="flex flex-row align-middle gap-x-2">
              ...
              <div className="mr-[25px] italic">{item.text}</div>
            </div>
          </div>
        ))}
      </Marquee>
